...
NORM_SD = BoundaryNorm(np.arange(0, 1.05, 0.05), CMAP_SD.N)
...
def multipanel(radar, outdir, dpi=90, debug=False, verbose=False):
    ...
    fig, axes = plt.subplots(nrows=len(SWEEPS), ncols=8, subplot_kw=subs)
    ...
    for i, sweep in enumerate(SWEEPS):
        ...
        qmc = _pcolormesh(
            radar, VDOP_CORR_FIELD, sweep=sweep+1, cmap=CMAP_VDOP,
        ...
        qmh = _pcolormesh(
            radar, DETECT_FIELD, sweep=sweep, cmap=CMAP_SD, norm=NORM_SD,
            fig=fig, ax=axes[i,7])

    # Create colour bars
    qm = [qma, qmb, qmc, qmd, qme, qmf, qmg, qmh]
