    ...
    for f in files:
	   ...
	   f, time_ordered='none', exclude_fields=EXCLUDE_FIELDS)
        ...
        for j, sweep in enumerate(SWEEPS):
            ...
            # (a) Raw reflectivity
            qma = _pcolormesh(
                radar, 'reflectivity', sweep, cmap=cmap_refl, 
                norm=norm_refl, ax=ax[0,j])

            # (b) Raw Doppler velocity
            qmb = _pcolormesh(
                radar, 'velocity', sweep, cmap=cmap_vdop, 
                norm=norm_vdop, ax=ax[1,j])

            # (c) Normalized coherent power
            qmc = _pcolormesh(
                radar, 'normalized_coherent_power', sweep, cmap=cmap_ncp,
                norm=norm_ncp, ax=ax[2,j])

            # (d) Correlation coefficient
            qmd = _pcolormesh(
                radar, 'cross_correlation_ratio', sweep, cmap=cmap_rhv,
                norm=norm_rhv, ax=ax[3,j])

	    # (e) Spectrum width
	    qme = _pcolormesh(
		radar, 'spectrum_width', sweep, cmap=cmap_sw, 
                norm=norm_sw, ax=ax[4,j])

	    # (f) Differential phase
	    qmf = _pcolormesh(
		radar, 'differential_phase', sweep, cmap=cmap_phi,
